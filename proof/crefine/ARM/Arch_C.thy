...
lemma setMRs_single:
  "setMRs thread buffer [val] = do _ \<leftarrow> asUser thread (setRegister register.R2 val);
       return 1
    od"
...
lemma decodeARMFrameInvocation_ccorres:
  ...
  apply (cinit' lift: invLabel_' length___unsigned_long_' cte_' current_extra_caps_' cap_' buffer_' call_'
   ...
   apply (rename_tac word rghts pg_sz mapdata calla buffera cap excaps cte length___unsigned_long invLabel)
   apply simp
   apply (rule ccorres_rhs_assoc)+
   apply csymbr+
  ...
  apply (rename_tac word rghts pg_sz mapdata calla buffera cap
